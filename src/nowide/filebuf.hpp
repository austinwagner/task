...
namespace nowide {
    ...
    protected:
        ...
        {
            nowide::cerr << "-- Is Null: " << (b==0) << std::endl;;
            if(b==0)
                return;
            if(e != 0)
                nowide::cerr << "-- Total: " << e - b <<" offset from start " << p - b << std::endl;
            else
                nowide::cerr << "-- Total: " << p - b << std::endl;
        ...
        void print_state()
        {
            nowide::cerr << "- Output:" << std::endl;
            print_buf(pbase(),pptr(),0);
            nowide::cerr << "- Input:" << std::endl;
            print_buf(eback(),gptr(),egptr());
            nowide::cerr << "- fpos: " << (file_ ? ftell(file_) : -1L) << std::endl;
        }
        ...
        struct print_guard
        {
            print_guard(basic_filebuf *p,char const *func)
            {
                self = p;
                f=func;
                nowide::cerr << "In: " << f << std::endl;
                self->print_state();
            }
            ~print_guard()
            {
                nowide::cerr << "Out: " << f << std::endl;
                self->print_state();
            }
            basic_filebuf *self;
            char const *f;
